...
CRITICAL_SECTION    myCriticalSection;//临界区对象
CONDITION_VARIABLE  myConditionVar;   //条件变量
...
DWORD WINAPI consumerThread(LPVOID param){
    ...
    while (true){
        ...
        while (tasks.empty()){
            ...
            //SleepConditionVariableCS 做了三件事：
            //1.释放锁
            //2.进入睡眠
            //3.被唤醒后重新获得锁
        ...
        pTask = tasks.front();
        tasks.pop_front();
        //SleepConditionVariableCS被唤醒后进入临界区，为了让其他线程有机会操作tasks，这里需要再次离开临界区
